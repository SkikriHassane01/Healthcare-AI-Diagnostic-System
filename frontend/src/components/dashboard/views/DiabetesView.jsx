...
const DiabetesView = ({ isDark, setActiveTab }) => {
  ...
  const handlePatientSelect = (patient) => {
    // Direct navigation to the diabetes assessment form for this patient
    navigate(`/patients/${patient.id}/diabetes-assessment`);
  };
  
  // Navigate to patient history
  const viewPatientHistory = (patient) => {
    // Direct navigation to the diabetes history for this patient
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="mb-4 flex border-b border-slate-300 dark:border-slate-700">
            <button 
              ...
              className={`py-2 px-4 ${selectedTab === 'overview' ? 
                (isDark ? 'text-emerald-400 border-b-2 border-emerald-400' : 'text-emerald-600 border-b-2 border-emerald-600') : 
          ...
          {selectedTab === 'overview' && (
            <div>
              ...
              <p className={`${isDark ? 'text-slate-300' : 'text-slate-600'} mb-4`}>
                Our AI model analyzes patient health data to assess the risk of diabetes, allowing for early intervention and personalized care planning.
              </p>
              ...
              <div className={`mb-6 p-4 rounded-lg ${isDark ? 'bg-slate-700' : 'bg-slate-50'}`}>
                ...
                <ul className={`${isDark ? 'text-slate-300' : 'text-slate-600'} list-disc list-inside space-y-1`}>
                  <li><span className="font-medium">Blood Glucose Level</span>: Measures blood sugar levels</li>
                  <li><span className="font-medium">HbA1c</span>: Indicates average blood glucose over 2-3 months</li>
                  <li><span className="font-medium">BMI</span>: Body mass index (weight/height²)</li>
                  <li><span className="font-medium">Age</span>: Risk increases with age</li>
                  <li><span className="font-medium">Medical History</span>: Hypertension, heart disease, etc.</li>
                  <li><span className="font-medium">Lifestyle</span>: Smoking history and other factors</li>
                </ul>
              </div>
                ...
                {patients.slice(0, 5).map(patient => (
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      <button className={`${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>
        ...
        <div className={`lg:col-span-2 ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} rounded-lg shadow-sm border p-4`}>
          <h3 className="font-semibold mb-4">Start Diabetes Assessment</h3>
          ...
          {loading ? (
            <div className="text-center py-8">
              <div className="inline-block w-8 h-8 border-4 border-t-transparent border-emerald-500 rounded-full animate-spin"></div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="flex-grow">
                      ...
                      <div className="flex gap-2 mt-3">
                        <button 
                          onClick={() => handlePatientSelect(patient)}
                          className={`flex-1 py-2 px-3 ${isDark ? 'bg-emerald-700 hover:bg-emerald-600' : 'bg-emerald-600 hover:bg-emerald-500'} text-white text-sm rounded transition-colors flex items-center justify-center`}
                        >
